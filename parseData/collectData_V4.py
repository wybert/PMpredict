...
class CollectData():
    def __init__(self, keyword, startTime, region, savedir, interval='50', fileNum='0', flag=True, begin_url_per = "http://s.weibo.com/weibo/"):
    ...
    def getURL(self):
        return self.begin_url_per+self.getKeyWord()+"&region=custom:"+self.region+"&xsort=time&timescope=custom:"+self.timescope+"&page="
    ...
    def download(self, url, fileNum, frompid=1,maxTryNum=4):
        ...
        mid_filter = set([])
        isCaught = False
        while hasMore and i < 53 and (not isCaught):
